...
public class W11_MapMinValue {
    ...
    public  static  int  minValue( Map<String,Integer> map  ) {
        ...
        SortedSet<Integer> sm = new TreeSet<>(map.values());//map.values() retrieves a collection of all the values from the map.
        //new TreeSet<>(map.values()) creates a TreeSet from these values.
        // A TreeSet is a SortedSet that stores elements in a sorted and ascending order.
        return sm.first();
    }
...
public class MapMinValue {
    ...
    public static void main(String[] args) {
        ...
        testMap.put("Aaron", 11);
        testMap.put("Baron", 5);
        testMap.put("Cydeo", 27);
        testMap.put("Canada", 100);

        System.out.println("Minimum value: " + findMinValue(testMap));
    }

    public static int findMinValue(Map<String, Integer> map) {
        // Initial value should be the maximum possible
        int minValue = Integer.MAX_VALUE;

        // Iterate through the map
        for (Integer value : map.values()) {
            // Update the minValue if a smaller value is found
            if (value < minValue) {
                minValue = value;
            }
        }
